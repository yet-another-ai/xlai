...
pub(crate) const OPENROUTER_RESPONSE_OUTPUT_METADATA_KEY: &str = "openrouter_response_output";
...
pub(crate) struct OpenRouterChatResponse {
    ...
    #[serde(default)]
    incomplete_details: Option<OpenRouterIncompleteDetails>,
}

impl OpenRouterChatResponse {
    pub(crate) fn into_core_response(self) -> Result<ChatResponse, XlaiError> {
        let (content, tool_calls) = openrouter_response_output_to_chat(&self.output)?;
...
pub(crate) struct OpenRouterErrorEnvelope {
    pub(crate) error: OpenRouterErrorDetail,
}
...
#[derive(Deserialize)]
pub(crate) struct OpenRouterIncompleteDetails {
    #[serde(default)]
...
pub(crate) fn attach_response_output_items(
    ...
    if !output.is_empty() {
        message.metadata.insert(
            OPENROUTER_RESPONSE_OUTPUT_METADATA_KEY.to_owned(),
...
pub(crate) fn response_output_items_from_message(message: &ChatMessage) -> Option<Vec<Value>> {
    message
        .metadata
        .get(OPENROUTER_RESPONSE_OUTPUT_METADATA_KEY)
        .and_then(|value| {
...
pub(crate) fn openrouter_response_output_to_chat(
    output: &[Value],
        ...
        match obj.get("type").and_then(Value::as_str) {
            Some("message") => {
                ...
                parts.extend(
                    content_items
                        .iter()
                        .filter_map(parse_openrouter_response_content_part),
                );
            }
            Some("function_call") => {
                tool_calls.push(parse_openrouter_function_call_item(obj)?);
            }
            _ => {}
        }
    }
...
fn parse_openrouter_response_content_part(value: &Value) -> Option<ContentPart> {
...
fn parse_openrouter_function_call_item(
...
mod tests {
    ...
    fn maps_prompt_cache_usage_details() {
        let parsed = serde_json::from_value::<OpenRouterChatResponse>(json!({
            "output": [],
            "status": "completed",
            "usage": {
                "prompt_tokens": 20,
