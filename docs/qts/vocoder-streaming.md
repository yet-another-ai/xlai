...
3. **Overlap-add** (`OverlapAddChunkDecoder` in `pipeline/vocoder_streaming.rs`): Each generated chunk may be decoded with the last _N_ frames of the previous chunk prepended (_N_ = overlap). The overlapping audio region is linearly crossfaded. The ONNX graph itself remains **stateless** per call.
...
See also: [Export and Hugging Face](/qts/export-and-hf-publish) for artifact layout and future stateful-vocoder notes.
