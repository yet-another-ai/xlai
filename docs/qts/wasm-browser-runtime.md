...
2. **Asset manifest:** Use a versioned manifest (see [Browser model manifest](/qts/wasm-model-manifest)) for fetch, cache, and integrity checks instead of a POSIX `model_dir` on disk.
