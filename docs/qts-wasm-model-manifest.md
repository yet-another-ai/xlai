...
Tokenizer / `config.json` are typically embedded in GGUF for this stack; if split artifacts are added later, extend `logical_name` enums in `xlai-qts-browser`.
...
See crate `xlai-qts-browser` (`QtsModelManifest`, `QtsModelFileEntry`) for serde-compatible structs and `validate_required_files`.
