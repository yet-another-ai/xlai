...
| `xlai-facade`                         | Internal (not on crates.io): **native-only** integration layer for `xlai-native` (default local `llama.cpp` wiring plus optional `qts`, Gemini + HTTP/local backends). `xlai-wasm` does not use this crate.                                                                                                                |
| `xlai-native`                         | Native Rust entrypoint: explicit re-exports and `gemini` submodule; uses `xlai-facade` for feature wiring. Local `llama.cpp` is included by default; enable optional `qts` for `QtsTtsModel`.                                                                                                                              |
